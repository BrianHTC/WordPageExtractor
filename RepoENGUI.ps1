...
function Append-Log([System.Windows.Forms.RichTextBox]$LogBox, [string]$Message) {
    if ($null -ne $LogBox -and -not $LogBox.IsDisposed) {
        ...
        if ($Message -eq "Done. Press the 'Close' button.") {
...
function Run-ExtractionFromGui([string]$InputPathText, [string]$RangesText, [string]$OutputFolderText, [string]$ModeText, [System.Windows.Forms.RichTextBox]$LogBox) {
    ...
    try {
        ...
        Append-Log $LogBox "Done. Press the 'Close' button."
...
function Show-Gui {
    ...
    $lblHint.Text = "Examples: 1-2 or 1-3,5,8-10"
    ...
    $radioC.Text = "C - Combined: put selected pages/ranges into one file"
    ...
    $radioS.Text = "S - Separate: split comma-separated ranges into different files"
    ...
    $txtOut.Location = New-Object System.Drawing.Point(130, 227)
    ...
    $btnOut.Text = "Browse..."
    $btnOut.Location = New-Object System.Drawing.Point(615, 225)
    ...
    $lblOutHint.Text = "Leave blank to create an output folder next to the source file."
    $lblOutHint.Location = New-Object System.Drawing.Point(130, 262)
    $lblOutHint.AutoSize = $true
    $lblOutHint.ForeColor = [System.Drawing.Color]::DimGray
    $form.Controls.Add($lblOutHint)

    $btnRun = New-Object System.Windows.Forms.Button
    $btnRun.Text = "Run extraction"
